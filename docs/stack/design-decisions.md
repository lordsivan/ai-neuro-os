...
> **Org & repos.** **`ai-neuro-stack`** is the GitHub **organization** holding all repos.
> Inside it: the **design / architecture source of truth** repo — **`ai-neuro-os`**
> (architecture & design only; owns the versioned `contracts/`) — and the **implementation**
> repo — **`ai-neuro-ba-app`** (the Phase-1 BA-facing app, then beyond). The two repos are
...
> *(Migration: this design currently lives in `lordsivan/ai-neuro-os` and moves to
> `ai-neuro-stack/ai-neuro-os` as canonical. Design-repo name assumed `ai-neuro-os`.)*
...
1. **Topology — one org, two repos, version the seam, don't merge.** **Decided.**
   **`ai-neuro-stack`** is the GitHub **organization**; both repos live under it:
   - **`ai-neuro-stack/ai-neuro-os`** — **design source of truth**; owns the architecture,
     the component specs, and the **`contracts/`** artifact, which it **semver-tags** on
     release.
   - **`ai-neuro-stack/ai-neuro-ba-app`** — the **implementation** (Phase-1 BA app, then beyond).
   The implementation **pins** a contract version (a published package
   `@ai-neuro-stack/contracts@X.Y.Z`, or a git submodule pinned to a release tag as the
...
All repos live under the **`ai-neuro-stack`** GitHub org.
...
4. **Conformance gate between every adjacent pair.** Architecture→Design = review /
   ...
   pinned contract.
5. **Repo mapping (org `ai-neuro-stack`).** The **`ai-neuro-os` repo** carries layers 1–2
   (Architecture + Design + the `contracts/` artifact); the **`ai-neuro-ba-app` repo**
   carries layers 3–4 (Implementation + Test). Both live under the `ai-neuro-stack` org. The
   **seam between the repos is the versioned contract** — exactly the Design↔Implementation
   boundary (mechanism in ADR-0003). No git merge crosses the repo boundary; the impl repo
   consumes a **pinned** contract version.
